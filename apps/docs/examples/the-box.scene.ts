...
export const scene = Scene.make(function* () {
	...
	// ── act I: arrival ──
	yield* Motion.tweenTo(ball, { x: 140 }, "900 millis", "easeOutCubic");
	...
	yield* Motion.tweenTo(ball, { x: 250 }, "450 millis", "easeInCubic");
	yield* arc(316, 186, GROUND - R, 460);
	yield* landOn(GROUND);
	yield* Motion.wait("300 millis");

	// attempt 2: back up further, run faster, jump higher —
	yield* Motion.tweenTo(ball, { x: 80 }, "500 millis", "easeInOutCubic");
	yield* Motion.wait("250 millis");
	yield* Motion.tweenTo(ball, { x: 290 }, "420 millis", "easeInQuad");
	// — and meet the wall, exactly at ledge height
	yield* Scene.all([
		Motion.tweenTo(ball, { x: LEDGE_X - R }, "260 millis"),
		Motion.tweenTo(ball, { y: LEDGE_TOP + 4 }, "260 millis", "easeOutQuad"),
	]);
	// the slow cartoon slide down the wall
	yield* Motion.tweenTo(ball, { y: GROUND - R }, "600 millis", "easeInSine");
	yield* squash(GROUND, R + 7, R - 7, 90);

	// dejection: it stays squashed and thinks about its life
	yield* squash(GROUND, R + 4, R - 5, 450);
	const dots = yield* Scene.instantiate(Shapes.Text, {
		text: "…",
		x: LEDGE_X - R,
		y: 205,
		fontSize: 22,
	...
	// ── act III: the idea ──
	const idea = yield* Scene.instantiate(Shapes.Text, {
		text: "!",
		x: LEDGE_X - R,
		y: 200,
		fontSize: 6,
	...
	yield* Motion.tweenTo(ball, { x: -40 }, "600 millis", "easeInCubic");
	...
	yield* Motion.tweenTo(ball, { x: 414 }, "400 millis", "easeOutCubic"); // to the star

	// the star celebrates — and so does everything around it
	const sparkle = (angle: number) =>
		Effect.gen(function* () {
			const s = yield* Scene.instantiate(Shapes.Circle, {
				x: 430,
				y: LEDGE_TOP - 12,
				radius: 3,
				fill: "#ff8906",
	...
	yield* Scene.fork(
		Effect.gen(function* () {
			yield* Motion.tweenTo(star, { radius: 13 }, "300 millis", "easeOutBack");
			yield* Scene.stagger(
