...
An array-backed particle emitter for effect-motion. A `ParticleField` is a single `Instance` — one node in the entity tree, advanced by one fiber, one party on the phaser — whose data holds a fixed-capacity buffer of many particles, so the cost on the frame barrier is independent of particle count. Authors configure per-particle properties as Tier-2 distribution config (uniform ranges sampled once at birth, plus age-driven over-life curves), and emit particles via burst (one-shot count), stream (rate per second), or fill (an evenly-spread, wrapping floating field). Each particle carries its own PRNG seeded from the runner's seeded `Random` service, keeping the field byte-identical across runs at a fixed draw order; a `simulate` animator advances the field one frame at a time, following the library's animator conventions.
