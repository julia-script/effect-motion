...
export const scene = Scene.make(
	function* () {
		...
		const custom = yield* Scene.instantiate("Text", { position: S.vec3({ x: 250, y: 120 }), text: "Custom fonts", fontSize: 48, fontFamily: pacifico, fillColor: Color.hex("#7f5af0"), textAnchor: "middle", baseline: "middle", opacity: 0 });
		...
		const plain = yield* Scene.instantiate("Text", { position: S.vec3({ x: 250, y: 190 }), text: "vs the default sans-serif", fontSize: 20, fillColor: Color.hex("#94a3b8"), textAnchor: "middle", baseline: "middle", opacity: 0 });
