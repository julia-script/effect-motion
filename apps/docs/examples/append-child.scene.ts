...
export const scene = Scene.make(
	function* () {
		...
		const platform = yield* Scene.instantiate("Group", {
			position: S.vec3({ x: -190, y: 0 }),
		});

		// a dot born at the root (not yet in the group), up in the corner
		...
		yield* Motion.moveTo(dot, { x: -190, y: 0 }, "700 millis", "easeOutCubic");
		...
		yield* platform.pipe(
			...
			Motion.moveTo({ x: -190 }, "1 second", "easeInOutCubic"),
		);
	},
