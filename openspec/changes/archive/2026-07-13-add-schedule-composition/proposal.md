...
- New `Scene.all(effects)`: the public counterpart to the low-level `Phaser.all` — plain lockstep parallel, no schedule.
- New `Scene.chain(effects, schedule?)`: sequential list composition; the schedule is consulted after each item *completes* (Effect-faithful: items never overlap) to pace the next start. The schedule bounds how many items run — items beyond its recursion limit are skipped.
- New `Scene.stagger(effects, schedule)`: the explicit overlap opt-in — staggered *starts* on the schedule, released effects run concurrently, resolves when all released effects finish. Same truncation semantics as chain.
...
- `scene-all`: public lockstep-parallel combinator (no schedule).
- `scene-chain`: sequential list composition paced by a schedule consulted after each item completes (no overlap).
- `scene-stagger`: explicit overlapping staggered starts on a schedule.
