...
## Purpose
The command shell shared by all `motion` subcommands: Effect Command API structure (subcommands, help/version), the single tagged CLI error type with its wrap-at-the-boundary rule, and terminal error reporting/exit codes.
