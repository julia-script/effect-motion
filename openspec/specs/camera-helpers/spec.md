...
## Purpose
The camera-directing vocabulary built on the point of interest: `lookAt`, `follow`, `orbit`/`orbitTo`, `dolly`/`dollyTo` — polymorphic targets (Instance / Effect / plain Position) with offsets, retargeted-tween re-aims, and the no-pair naming rule for target-naming verbs.
