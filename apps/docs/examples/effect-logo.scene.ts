...
const CENTER_X = 0;
const CENTER_Y = 40; // stack midpoint in world y
const SPACING = 30; // vertical gap between plates
const PITCH = 0.55; // camera pitch (radians); sin ≈ 0.52 ≈ 2:1 diamonds

export const scene = Scene.make(
	function* () {
		// Rects are center-anchored, so each diamond sits centered on its
		// position. All plates park above the frame and spring down into
		// the stack.
		const plate = (style: {
			fill: Color.Color;
			stroke?: Color.Color;
			...
			Scene.instantiate("Rect", {
				position: S.vec3({ x: CENTER_X, y: CENTER_Y + 340 }),
