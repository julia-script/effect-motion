...
const GROUND = -110; // floor surface
const LEDGE_X = 90; // left face of the ledge
const LEDGE_TOP = 0;
const R = 16; // the ball at rest

export const scene = Scene.make(
	function* () {
		// ── the world ── (Rects are center-anchored)
		yield* Scene.instantiate("Rect", {
			position: S.vec3({ x: 0, y: GROUND - 20 }),
			width: 500,
			height: 40,
			fillColor: Color.hex("#efeeea"),
		});
		yield* Scene.instantiate("Rect", {
			position: S.vec3({ x: LEDGE_X + 80, y: (LEDGE_TOP + GROUND) / 2 }),
			width: 160,
			height: LEDGE_TOP - GROUND,
			fillColor: Color.hex("#c9d1e5"),
		});
		const star = yield* Scene.instantiate("Circle", {
			position: S.vec3({ x: 180, y: LEDGE_TOP + 12 }),
			radius: 8,
			fillColor: Color.hex("#ff8906"),
		});
		const box = yield* Scene.instantiate("Rect", {
			position: S.vec3({ x: -310, y: GROUND + 28 }), // waiting in the wings
			width: 56,
			height: 56,
			fillColor: Color.hex("#2cb67d"),
		});
		const ball = yield* Scene.instantiate("Ellipse", {
			position: S.vec3({ x: -280, y: GROUND + R }),
		...
		// ── act I: arrival ──
		yield* Motion.moveTo(ball, { x: -110 }, "900 millis", "easeOutCubic");
		yield* squash(GROUND, R + 1.5, R - 1.5, 300); // a small breath
		yield* squash(GROUND, R, R, 300);
		// it sees the star: a hop of intent
		yield* arc(-100, GROUND + R + 26, GROUND + R, 350);
		yield* Motion.wait("250 millis");

		// ── act II: try harder ──
		// attempt 1: an honest jump, an honest shortfall
		yield* Motion.moveTo(ball, { x: 0 }, "450 millis", "easeInCubic");
		yield* arc(66, LEDGE_TOP - 36, GROUND + R, 460);
		yield* landOn(GROUND);
		yield* Motion.wait("300 millis");

		// attempt 2: back up further, run faster, jump higher —
		yield* Motion.moveTo(ball, { x: -170 }, "500 millis", "easeInOutCubic");
		yield* Motion.wait("250 millis");
		yield* Motion.moveTo(ball, { x: 40 }, "420 millis", "easeInQuad");
		...
		const dots = yield* Scene.instantiate("Text", {
			position: S.vec3({ x: LEDGE_X - R, y: -55 }),
			text: "…",
			fontSize: 22,
			fillColor: Color.hex("#9490a6"),
			opacity: 0,
			textAnchor: "middle",
		});
		...
		// ── act III: the idea ──
		const idea = yield* Scene.instantiate("Text", {
			position: S.vec3({ x: LEDGE_X - R, y: -50 }),
			text: "!",
			fontSize: 6,
			fillColor: Color.hex("#ff8906"),
			opacity: 0,
		...
		yield* Motion.moveTo(ball, { x: -290 }, "600 millis", "easeInCubic");
		yield* Motion.wait("400 millis");

		// …and returns, pushing the answer. Leaning into the work.
		yield* Scene.all([
			// box center lands 28 short of the wall: its right face kisses the ledge
			Motion.moveTo(box, { x: LEDGE_X - 28 }, "1500 millis", "easeInOutSine"),
			Motion.moveTo(
				ball,
				{ x: LEDGE_X - 56 - R },
				"1500 millis",
		...
		yield* arc(134, LEDGE_TOP + R + 42, LEDGE_TOP + R, 430); // onto the ledge
		yield* landOn(LEDGE_TOP);
		yield* Motion.moveTo(ball, { x: 164 }, "400 millis", "easeOutCubic"); // to the star

		// the star celebrates — and so does everything around it
		const sparkle = (angle: number) =>
			Effect.gen(function* () {
				const s = yield* Scene.instantiate("Circle", {
					position: S.vec3({ x: 180, y: LEDGE_TOP + 12 }),
					radius: 3,
					fillColor: Color.hex("#ff8906"),
				});
		...
		yield* arc(164, LEDGE_TOP + R + 18, LEDGE_TOP + R, 280);
		yield* arc(164, LEDGE_TOP + R + 12, LEDGE_TOP + R, 240);

		// ── the moral ──
		const caption = yield* Scene.instantiate("Text", {
			position: S.vec3({ x: 0, y: 80 }),
