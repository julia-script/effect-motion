# resvg-rasterization Specification

## Purpose
Turn SVG document strings — typically the string sink's output — into PNGs via resvg, as composable Effect functions in the Node-only `@effect-motion/export` package. Rasterization is an export tool, not a renderer: renderers produce portable descriptions; export tools consume them.
