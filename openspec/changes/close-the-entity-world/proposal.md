...
- **BREAKING** — `Group`'s 2D affine is removed: `TransformMatrix`, `TransformOperation`, the transform-operation DSL, `identityTransform`, and `multiplyTransforms`. Group composition uses the same TRS transform as every other entity. Shear is no longer expressible, which costs nothing: the ops→affine DSL was never wired up (its only test is skipped, annotated "never implemented") and the renderer never threaded a Group's affine into child coordinates. This removes dead code rather than a capability, and three.js `Object3D` is TRS-native so the model maps 1:1.
