# motion-renderer Delta Specification
...
**Reason**: Projection happens on the GPU via the camera's projection matrix; there is no per-paint CPU screen affine, quad, or transform call, and near-plane behavior (the old `scale <= 0` cull) is handled by GPU clipping.
**Migration**: Subsumed by the coordinate-mapping and billboard requirements above.
