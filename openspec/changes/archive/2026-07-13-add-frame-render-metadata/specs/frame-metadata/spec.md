# frame-metadata Specification (delta)
...
The generic renderer family (`Renderer.make`) SHALL pass the frame's `{ frameRate, width, height }` to the sink's render function, so custom sinks can size or time their output from the frame alone.
