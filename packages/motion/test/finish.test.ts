...
describe("Scene.finish", () => {
	it("releases awaiters immediately while the tail keeps playing", async () => {
		const frames = await collectFrames(function* () {
			...
			const handle = yield* Scene.fork(
				Effect.gen(function* () {
					yield* Motion.move(b, { x: 0 }, { x: 50 }, "0.5 seconds");
					yield* Scene.finish;
					// tail: 30 more frames, nobody waits for these
					yield* Motion.move(b, { x: 50 }, { x: 100 }, "0.5 seconds");
				}) as never,
			);
		...
		expect(frames[35]?.[0]?.position.x).toBeGreaterThan(0); // a moving…
		expect(frames[35]?.[1]?.position.x).toBeGreaterThan(50); // …while b's tail moves
		expect(frames.at(-1)?.[1]?.position.x).toBe(100); // tail completed naturally
	});
		...
		const frames = await collectFrames(function* () {
			...
			yield* Scene.fork(
				Effect.gen(function* () {
					yield* Motion.move(b, { x: 0 }, { x: 50 }, "0.5 seconds");
					yield* Scene.finish;
					yield* Motion.moveTo(b, { x: 500 }, "10 seconds"); // long tail
				}) as never,
			);
			yield* Motion.move(a, { x: 0 }, { x: 100 }, "1 second");
		});
		// bounded by the 60-frame body, not the 600-frame tail
		expect(frames).toHaveLength(61);
		const lastB = frames.at(-1)?.[1]?.position.x;
		...
		const frames = await collectFrames(function* () {
			...
			yield* Scene.fork(
				Effect.gen(function* () {
					yield* Motion.move(b, { x: 0 }, { x: 50 }, "0.5 seconds");
					yield* Scene.finish;
					yield* Motion.moveTo(b, { x: 500 }, "10 seconds");
				}) as never,
			);
			// un-finished fork: holds the scene open for 90 frames
			yield* Scene.fork(Motion.move(c, { x: 0 }, { x: 100 }, "1.5 seconds"));
		});
		expect(frames).toHaveLength(91);
		// the finished fork's tail animated through the drain
		expect(frames[80]?.[1]?.position.x).toBeGreaterThan(
			frames[40]?.[1]?.position.x,
		);
	});
	...
	it("finish is idempotent; finish-then-complete decrements once", async () => {
		const frames = await collectFrames(function* () {
			...
			yield* Scene.fork(Motion.move(c, { x: 0 }, { x: 100 }, "0.5 seconds"));
		});
	...
	it("completion implies finish", async () => {
		const frames = await collectFrames(function* () {
			...
			yield* Motion.move(a, { x: 0 }, { x: 100 }, "166 millis");
		});
		expect(frames).toHaveLength(42); // 30 + 1 wake frame + 10 + settle
		expect(frames[29]?.[0]?.position.x).toBe(0); // a waited for b's completion
	});
	...
	it("a parent bounds a tail by interrupting the handle's fiber", async () => {
		...
		expect(frames[55]?.[1]?.position.x).toBe(frames[42]?.[1]?.position.x);
	});
});
