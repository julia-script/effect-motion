...
## Purpose
Scene-level font declaration and the per-environment loading contracts. Scenes declare the fonts their text depends on via the `Fonts` annotation; the runtime never reads it (the engine cannot measure text, so fonts cannot affect frame data). Each consumer honors the declaration its own way: the browser player loads `url` sources through the FontFace API, export tools feed `path` sources to resvg.
